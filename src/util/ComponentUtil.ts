...
class ComponentUtil {
  private static REFRESH_ICON_START_TOP = 40;
  private static REFRESH_TRIGGER_DISTANCE = 100;

  public enablePullToRefresh(target: DomNode, refresh: () => void) {
    target.onDom("touchstart", (event: TouchEvent) => {
      if (target.domElement.scrollTop !== 0) return;
      ...
      const onTouchEnd = () => {
        if (moved && move > ComponentUtil.REFRESH_TRIGGER_DISTANCE) refresh();
        refreshIcon.delete();
