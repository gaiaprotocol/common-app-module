...
const normalizePathname = (pathname: string) => decodeURIComponent(pathname.endsWith("/") ? pathname.slice(0, -1).substring(1) : pathname.substring(1));

const matchPattern = (uriParts: string[], pattern: string[], excludes: string[], params: ViewParams) =>
    pattern.some((pat) => URIParser.match(uriParts, pat.split("/"), params)) && !excludes.some((exclude) => URIParser.match(uriParts, exclude.split("/")));
