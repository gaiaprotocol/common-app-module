...
export default class SupabaseService extends EventContainer {
    ...
    async safeSelect(build, fetchLimit = this.fetchLimit, selectQuery = this.selectQuery) {
        const data = await Supabase.safeFetch(this.tableName, (b) => build(fetchLimit === -1
    ...
    async safeExists(build) {
        const { data, error } = await build(Supabase.client.from(this.tableName).select("*").limit(1));
        if (error)
            throw error;
    ...
    async safeUpsert(build, data) {
        const { error } = await build(Supabase.client.from(this.tableName).upsert(data));
        if (error)
            throw error;
    }
